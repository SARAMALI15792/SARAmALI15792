...
def build(rows, total_dur, width, height, title):
    cols = len(rows[0])
    chrome, (bx, by, bw, bh) = frame(width, height, title, uid="a")

    # one row is held back for the cursor line, so the art never sits on it
    size = min(bw / (cols * 0.6), bh / (len(rows) + 1))
    cw = size * 0.6
    x0 = bx + (bw - cols * cw) / 2
    y0 = by + (bh - (len(rows) + 1) * size) / 2
    ...
    for i, row in enumerate(rows):
        y = y0 + (i + 1) * size
        defs.append(
            f'<clipPath id="r{i}"><rect x="{x0:.1f}" y="{y - size:.1f}" '
            f'height="{size:.1f}" width="0">'
            f'<animate attributeName="width" from="0" to="{cols * cw:.1f}" '
        ...
        body.append(
            f'<text x="{x0:.1f}" y="{y:.1f}" clip-path="url(#r{i})" '
            f'textLength="{len(row) * cw:.1f}" lengthAdjust="spacingAndGlyphs" '
            f'xml:space="preserve">{html.escape(row)}</text>'
        )

    # blinking cursor parks on the line below the portrait once it finishes
    cursor = (
        f'<rect x="{x0:.1f}" y="{y0 + len(rows) * size:.1f}" width="{cw:.1f}" '
        f'height="{size:.1f}" fill="{ACCENT}" opacity="0">'
    ...
    return (
        ...
        f"{chrome}<defs>{''.join(defs)}</defs>"
        f'<g font-size="{size:.2f}" fill="{FG}">'
...
def main():
    ...
    p.add_argument("--title", default="saram@github: ~$ ./portrait.sh")
    a = p.parse_args()

    rows = to_rows(a.photo, a.cols, a.contrast)
    with open(a.out, "w") as f:
        f.write(build(rows, a.duration, a.width, a.height, a.title))
